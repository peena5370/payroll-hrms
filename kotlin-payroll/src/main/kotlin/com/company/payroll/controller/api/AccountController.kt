...
class AccountController(@Autowired private val systemAccountService: SystemAccountService) {
  ...
  @GetMapping
  fun listAccount(@RequestParam(value = "page", required = true) page: Int, @RequestParam(value = "size", required = true) offset: Int): ResponseEntity<PageInfo<Account>> {
    return ResponseEntity.ok(systemAccountService.list(page, offset))
  }

  @Operation(summary = "Get account info by id")
  @GetMapping("/{id}")
  fun getById(@Parameter(description = "Account id") @PathVariable("id") id: Int): ResponseEntity<Account?> {
    return ResponseEntity.ok(systemAccountService.findById(id))
  }

  @Operation(summary = "Register new account")
  @RequestBody(content = [Content(mediaType = "application/json", schema = Schema(implementation = Account::class), examples = [ExampleObject(name = "Example 1", value = "valueOne")])])
  @PostMapping
  @Throws(NoSuchAlgorithmException::class)
  fun insert(@RequestBody account: Account): ResponseEntity<String> {
    val (aId, username, password, secretkey, dateCreated, dateModified, lastLogin, lastAttempt, accountStatus, imgPath, staffId) = systemAccountService.insert(account)
  ...
  @RequestBody(required = true, content = [Content(mediaType = "application/json", schema = Schema(implementation = Account::class), examples = [ExampleObject(name = "Example 1", value = "VALUE_THREE")])])
  @PutMapping("/{id}/password")
  @Throws(NoSuchAlgorithmException::class)
  fun listUpdatePassword(@RequestBody account: Account): ResponseEntity<String> {
    val (aId, username, password, secretkey, dateCreated, dateModified, lastLogin, lastAttempt, accountStatus, imgPath, staffId) = systemAccountService.updateListPassword(account)
        ?: return ResponseEntity.status(HttpStatus.FORBIDDEN).body<String?>("password update failed")
    return ResponseEntity.ok("update success")
  }

  @Operation(summary = "Update account.")
  @RequestBody(required = true, content = [Content(mediaType = "application/json", schema = Schema(implementation = Account::class), examples = [ExampleObject(name = "Example 1", value = "VALUE_TWO")])])
  @PutMapping("/{id}")
  fun update(@RequestBody account: Account): ResponseEntity<String> {
    val (aId, username, password, secretkey, dateCreated, dateModified, lastLogin, lastAttempt, accountStatus, imgPath, staffId) = systemAccountService.update(account)
